...
% globals
base_Dir = '...\Addtional_ROI_Analysis';
...
toc
%% create ROIs from freeroi Toolbox assests

    function[] = get_ROIs()
        % Download freeroi-win-0.2.3-beta here: https://sourceforge.net/projects/freeroi/files/?source=navbar
        freeroiDir = 'F:\bigbipsy2\fmecklenbrauck\09_WS24_25\Movie-HINTS Paper\Review\Addtional_ROI_Analysis\freeroi-win-0.2.3-beta\freeroi';
        
        gunzip(fullfile(freeroiDir, 'data', 'label', 'face-object', 'facelabel.nii.gz'),...
            base_Dir);
        
        
        labels = {'r_OFA'; 'l_OFA';'r_pFus';'l_pFus';...
            'r_pcSTS';'l_pcSTS';'r_pSTS';'l_pSTS';...
            'r_aFus';'l_aFus';'r_aSTS';'l_aSTS'};
        
        outDir = fullfile(base_Dir,roiDirs{1});
        if ~exist(outDir, 'dir')
            mkdir(outDir);
        end
        
        for i = [5:8,11,12]
            jobs{1}.spm.util.imcalc.input = {fullfile(base_Dir,'facelabel.nii')};
            jobs{1}.spm.util.imcalc.output = [labels{i},'.nii'];
            jobs{1}.spm.util.imcalc.outdir = {outDir};
            jobs{1}.spm.util.imcalc.expression = ['i1 ==', num2str(i)];
        ...
        for h = 1:numel(hierarchy)
            ...
            for d = 1:numel(duration)
                ...
                for i = 1:number_subjects
                    ...
                    output(:, non_i) = y';
                    save(fullfile(study_dir,subjects(i,:),[subjects(i,:),'_ISCpair_STS_ROIs_',...
                    ...
                    writetable(outputTable,...
                        fullfile(study_dir,subjects(i,:),[subjects(i,:),'_ISCpair_STS_ROIs_',...
